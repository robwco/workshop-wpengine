 ...
 * Version: $Id: integration.shoppingcart.infusionsoft.php 1928 2013-12-26 15:31:25Z mike $
 */
...
if (wlm_arrval($_GET, 'cart') == $__index__) {
	if (!$__INTERFACE__) {
		...
		// save API Key
		if (wlm_arrval($_POST, 'isapikey')) {
			$_POST['isapikey'] = trim(wlm_arrval($_POST, 'isapikey'));
			$wpmy = trim(preg_replace('/[^A-Za-z0-9]/', '', $_POST['isapikey']));
			if ($wpmy == $_POST['isapikey']) {
				$isapikey = $this->GetOption('isapikey');
				$isapikey = $isapikey ? $isapikey: "";
				if($isapikey != $wpmy){
					$this->SaveOption('isapikey',$wpmy);
				...
				echo "<div class='error fade'>" . __('<p><b>Error:</b> API Key may only contain letters and numbers.</p>', 'wishlist-member') . "</div>";
			}
		}
		...
		<blockquote>
			<form method="post">
				...
				<p class="submit">
					&nbsp;&nbsp;<input type="submit" class="button-secondary" value="<?php _e('Save API Connection','wishlist-member'); ?>" />
				</p>
			</form>
