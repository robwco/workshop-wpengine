 ...
 * @version $Rev: 2017 $
 * $LastChangedBy: mike $
 * $LastChangedDate: 2014-02-07 10:52:34 -0500 (Fri, 07 Feb 2014) $
 */
if (!defined('ABSPATH'))
	die();
if (!class_exists('WishListMemberPluginMethods')) {
	...
	class WishListMemberPluginMethods extends WishListMemberDBMethods {
		...
		function ScheduleToLevel($action, $level, $member_ids = null, $date) {
			$wpm_levels = $this->GetOption('wpm_levels');
			$todays_date = strtotime(date("Y-m-d"));
			$sched_date = date("Y-m-d", strtotime($date));
			$message = '';
			$meta_name = '';
...
STRING;
		...
		function Backup_Generate() {
			...
			foreach ($tables AS $table) {
				...
				//while($out=$wpdb->get_row("SELECT * FROM `{$table}`",ARRAY_A,$offset++)){
				$r = mysql_query("SELECT * FROM `{$table}`");
				while ($out = mysql_fetch_assoc($r)) {
