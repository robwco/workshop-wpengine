...
function crb_load_widgets() {
	...
	register_widget('ThemeWidgetMustRead');
}
...
class ThemeWidgetImage extends Carbon_Widget {
	...
	function ThemeWidgetImage() {
		$this->setup('Theme Widget - Image with Link', 'Displays an image along with an optional link.', array(
			Carbon_Field::factory('image', 'image')->help_text('Maximum width 300px. Larger images will automatically be cropped and resized.'),
	...
	function front_end($args, $instance) {
		...
		<img src="<?php echo wpthumb( $image, array( 'width' => 300, 'crop' => true, 'crop_from_position', 'center,center' ) ); ?>" alt="" />
		<?php if ( $link ) echo '</a>'; ?>
...
class ThemeWidgetTestimonial extends Carbon_Widget {
	...
	function front_end($args, $instance) {
		...
		<?php if ( $image ) : ?>
		<img src="<?php echo wpthumb( $image, array( 'width' => 130, 'height' => 130, 'crop' => true, 'crop_from_position', 'center,center' ) ); ?>" alt="" />
		<?php endif; ?>
 ...
 * A testimonial widget
 */
class ThemeWidgetMustRead extends Carbon_Widget {
	protected $form_options = array(
		'width' => 500
	);	
	 ...
	 * Register widget function. Must have the same name as the class
	 */
	function ThemeWidgetMustRead() {
		$this->setup('Theme Widget - Must Read', 'Displays a group of manually selected posts.', array(
			Carbon_Field::factory('relationship', 'posts')
				->set_post_type('post'),
			Carbon_Field::factory('checkbox', 'display_thumbnails', 'Display thumbnails')
	...
	function front_end($args, $instance) {
		...
		<?php foreach( $posts as $post ) : ?>
			...
			<li>			
				<?php if ( $display_thumbnails ) : $thumbnail_url = wp_get_attachment_image_src( get_post_thumbnail_id( $post ) ); ?>				
				<img src="<?php echo wpthumb( $thumbnail_url[0], array( 'width' => 125, 'height' => 100, 'crop' => true, 'crop_from_position', 'center,center' ) ); ?>" alt="" />
				<?php endif; ?>			
