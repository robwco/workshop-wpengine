...
Modified for the ThemeBeans at http://www.themebeans.com
*/

// take the content of a contact-form shortcode and parse it into a list of field types
function radium_contact_form_parse( $content ) {
	// first parse all the contact-field shortcodes into an array
	global $radium_contact_form_fields, $radium_form_form;
	$radium_contact_form_fields = array();

	$out = do_shortcode( $content );
	
	if ( empty($radium_contact_form_fields) || !is_array($radium_contact_form_fields) ) {
		...
		if ( 'yes' == strtolower($radium_form_form->show_subject) ) {
			$default_form .= '
...
function radium_contact_form_render_field( $field ) {
	global $contact_form_last_id, $contact_form_errors, $radium_contact_form_fields, $current_user, $user_identity;
	...
	if ( $field['type'] == 'email' ) {
		...
		$r .= "\t\t<label for='".esc_attr($field_id)."' class='radium_form-field-label ".esc_attr($field['type']) . ( contact_form_is_error($field_id) ? ' form-error' : '' ) . "'>" . htmlspecialchars( $field['label'] ) . ( $field['required'] ? '<span class="required">&nbsp;*</span>' : '' ) . "</label>\n";
		...
		$r .= "\t\t<label class='".esc_attr($field['type']) . ( contact_form_is_error($field_id) ? ' form-error' : '' ) . "'>\n";
		$r .= "\t\t<input type='checkbox' name='".esc_attr($field_id)."' value='".__( 'Yes', 'radium' )."' class='".esc_attr($field['type'])."' ".( $field_value ? "checked='checked' " : "")." /> \n";
...
function radium_contact_form_validate_field( $field ) {
	...
	if ( $field['required'] && $field['type'] == 'email' ) {
		if ( !is_email( $field_value ) ) {
			...
			$contact_form_errors->add( $field_id, sprintf( __( 'Hey now... a valid email address is required!', 'radium' ), $field['label'] ) );
		}
		...
		$contact_form_errors->add( $field_id, sprintf( __( 'Woah there... The "%s" field is required!', 'radium' ), $field['label'] ) );
	}
...
function radium_contact_form_field( $atts, $content, $tag ) {
	global $radium_contact_form_fields, $contact_form_last_id, $radium_form_form;
	...
	if ( $field['type'] == 'subject' && is_null($field['default']) )
		$field['default'] = $radium_form_form->subject;
	...
	if ( empty($id) ) {
		...
		$i = 0;
		$max_tries = 12;
		while ( isset( $radium_contact_form_fields[ $id ] ) ) {
			$i++;
	...
	$radium_contact_form_fields[ $id ] = $field;
	
	if ( isset( $_POST['contact-form-id'] ) && $_POST['contact-form-id'] == $contact_form_last_id )
		radium_contact_form_validate_field( $field );
	
	return radium_contact_form_render_field( $field );
}

add_shortcode('contact-field', 'radium_contact_form_field');


function radium_contact_form_shortcode( $atts, $content ) {
	global $post;
	...
	extract( shortcode_atts( array(
		...
		'widget' => 0 //This is not exposed to the user. Works with radium_contact_form_widget_atts
	), $atts ) );
	...
	global $wp_query, $radium_form_form, $contact_form_errors, $contact_form_values, $user_identity, $contact_form_last_id, $contact_form_message;
	
	// used to store attributes, configuration etc for access by contact-field shortcodes
	$radium_form_form = new stdClass();
	$radium_form_form->to = $to;
	$radium_form_form->subject = $subject;
	$radium_form_form->show_subject = $show_subject;
	...
	$body = radium_contact_form_parse( $content );

	$r = "<div id='contact-form-$id'>\n";
	
		
	$action = apply_filters( 'radium_form_contact_form_form_action', get_permalink( $post->ID ) . "#contact-form-$id", $post, $id );
	...
	if ( is_wp_error( $contact_form_errors ) && $errors = (array) $contact_form_errors->get_error_codes() ) {
		$r .= "<div class='form-error'><ul class='form-errors'>\n";
		foreach ( $contact_form_errors->get_error_messages() as $message )
			$r .= "\t<li class='form-error-message animated BeanFadeOut'>$message</li>\n";
		$r .= "</ul>\n</div>\n\n";
	}
	...
	$message_sent = radium_contact_form_send_message( $to, $subject, $widget );
	...
	if ( is_wp_error( $contact_form_errors ) && $errors = (array) $contact_form_errors->get_error_codes() ) :
		...
		$r_success_message = "<h3>" . __( 'Message Sent', 'radium' ) . "</h3>\n\n";
		$r_success_message .= wp_kses($contact_form_message, array('br' => array(), 'blockquote' => array()));

		$r .= apply_filters( 'radium_form_contact_form_success_message', $r_success_message );
...
add_shortcode( 'contact-form', 'radium_contact_form_shortcode' );

function radium_contact_form_send_message( $to, $subject, $widget ) {
	global $post;
	...
	global $radium_contact_form_fields, $contact_form_message;
	...
	foreach ( $radium_contact_form_fields as $id => $field ) {
...
*/
	...
	$spam = '';
	$akismet_values = radium_contact_form_prepare_for_akismet( $contact_form_values );
	...
	$message .= __( "Contact Form URL:", 'radium' ) . " " . get_permalink( $post->ID ) . "\n";
	...
	global $do_radium_form_insert;
	$do_radium_form_insert = TRUE;
	add_filter( 'wp_insert_post_data', 'radium_form_insert_filter', 10, 2 );
	...
	remove_filter( 'wp_insert_post_data', 'radium_form_insert_filter' );
	$do_radium_form_insert = FALSE;
	...
	do_action( 'radium_form_pre_message_sent', $post_id, $all_values, $extra_values );

	# schedule deletes of old spam feedback
	if ( !wp_next_scheduled( 'radium_form_scheduled_delete' ) ) {
		wp_schedule_event( time() + 250, 'daily', 'radium_form_scheduled_delete' );
	}

	if ( $is_spam !== TRUE )
		return wp_mail( $to, "{$spam}{$subject}", $message, $headers );
	elseif ( apply_filters( 'radium_form_still_email_spam', FALSE ) == TRUE )
		return wp_mail( $to, "{$spam}{$subject}", $message, $headers );
...
function radium_contact_form_prepare_for_akismet( $form ) {
...
// submit an array to Akismet. If you're accepting a new item via $_POST, run it through radium_contact_form_prepare_for_akismet() first
function radium_contact_form_is_spam_akismet( $form ) {
	...
	if ( 'true' == trim( $response[1] ) ) // 'true' is spam
		$result = true;
	return apply_filters( 'radium_contact_form_is_spam_akismet', $result, $form );
}

// submit a comment as either spam or ham
// $as should be a string (either 'spam' or 'ham'), $form should be the comment array
function radium_contact_form_akismet_submit( $as, $form ) {
	global $akismet_api_host, $akismet_api_port;
...
function radium_contact_form_widget_atts( $text ) {
	static $widget = 0;
...
add_filter( 'widget_text', 'radium_contact_form_widget_atts', 0 );

function radium_contact_form_widget_shortcode_hack( $text ) {
...
function radium_contact_form_init() {
	if ( function_exists( 'akismet_http_post' ) ) {
		add_filter( 'contact_form_is_spam', 'radium_contact_form_is_spam_akismet', 10 );
		add_action( 'contact_form_akismet', 'radium_contact_form_akismet_submit', 10, 2 );
	}
	if ( !has_filter( 'widget_text', 'do_shortcode' ) )
		add_filter( 'widget_text', 'radium_contact_form_widget_shortcode_hack', 5 );

	// custom post type we'll use to keep copies of the feedback items
	register_post_type( 'feedback', array(
		'labels'            => array(
			'name'               => __( 'Responses', 'radium' ),
			'singular_name'      => __( 'Responses', 'radium' ),
			'search_items'       => __( 'Search', 'radium' ),
			'not_found'          => __( 'No responses found', 'radium' ),
			'not_found_in_trash' => __( 'No responses found', 'radium' )
		),
		'show_ui'           => TRUE,
...
add_action( 'init', 'radium_contact_form_init' );

/**
 * Add a contact form button to the post composition screen
 */
add_action( 'media_buttons', 'radium_form_media_button', 999 );
function radium_form_media_button( ) {
	global $post_ID, $temp_ID;
	$iframe_post_id = (int) (0 == $post_ID ? $temp_ID : $post_ID);
	$title = esc_attr( __( 'Bean Form Builder', 'radium' ) );
	$plugin_url = esc_url( RADIUM_FORMS_URL );
	$site_url = admin_url( "/admin-ajax.php?post_id=$iframe_post_id&amp;radium_form=form-builder&amp;action=radium_form_form_builder&amp;TB_iframe=true&amp;width=768" );
...
if ( !empty( $_GET['radium_form'] ) && $_GET['radium_form'] == 'form-builder' ) {
	add_action( 'parse_request', 'radium_parse_wp_request' );
	add_action( 'wp_ajax_radium_form_form_builder', 'radium_parse_wp_request' );
}

function radium_parse_wp_request( $wp ) {
	radium_display_form_view( );
	exit;
}

function radium_display_form_view( ) {
	require_once RADIUM_FORMS_DIR . '/form-view.php';
}



function radium_form_insert_filter( $data, $postarr ) {
	global $do_radium_form_insert;

	if ( $do_radium_form_insert === TRUE ) {
...
add_action( 'radium_form_scheduled_delete', 'radium_form_delete_old_spam' );
function radium_form_delete_old_spam() {
	global $wpdb;

	$radium_form_delete_limit = 100;
	...
	$sql = $wpdb->prepare( "
		...
		LIMIT %d
	", $now_gmt, $radium_form_delete_limit );
	...
	if ( apply_filters( 'radium_form_optimize_table', ( $random_number == 11 ) ) ) {
		$wpdb->query( "OPTIMIZE TABLE $wpdb->posts" );
	}

	# if we hit the max then schedule another run
	if ( count( $post_ids ) >= $radium_form_delete_limit ) {
		wp_schedule_single_event( time() + 700, 'radium_form_scheduled_delete' );
	}
}