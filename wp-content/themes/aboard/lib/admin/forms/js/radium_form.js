...
FB.ContactForm = function() {
	...
	var radium_formNewCount = 0; // increment for new fields
	...
	function addField () {
		try {
			radium_formNewCount++;
			if (radium_formNewCount <= maxNewFields) {
				// Add to preview
				jQuery('#fb-extra-fields').append('<div id="fb-new-field' + radium_formNewCount + '" fieldid="' + radium_formNewCount + '" class="fb-new-fields"><div class="fb-fields"><div id="' + radium_formNewCount + '" class="fb-remove"></div><label fieldid="' + radium_formNewCount + '" for="fb-field' + radium_formNewCount + '"><span class="label-text">' + GrunionFB_i18n.newLabel + '</span> </label><input type="text" id="fb-field' + radium_formNewCount + '" disabled="disabled" /></div></div>');
				// Add to form object
				fbForm.fields[radium_formNewCount] = {
				...
				jQuery('#fb-field-id').val(radium_formNewCount);
	...
	return {
		...
		init: function () {
			...
			var data = {
				action: 'radium_form_shortcode_to_json',
