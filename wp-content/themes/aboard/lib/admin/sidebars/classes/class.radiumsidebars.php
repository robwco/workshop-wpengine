...
class Radium_Sidebars {
	...
	public function register_post_type_columns () {
		...
		if ( $post_type != '' && post_type_supports( $post_type, 'radium' ) ) {
	...
	public function register_post_type () {	
		...
		$page 		= 'themes.php';
		$singular 	= __( 'Widget Area', 'radium' );
		$plural 	= __( 'Widget Areas', 'radium' );
		...
		$labels = array(
			'name' 					=> _x( 'Widget Areas', 'post type general name', 'radium' ),
			'singular_name' 		=> _x( 'Widget Area', 'post type singular name', 'radium' ),
			'add_new' 				=> _x( 'Add New', 'Sidebar Area', 'radium' ),
			'add_new_item' 			=> sprintf( __( 'Add New %s', 'radium' ), $singular ),
			'edit_item' 			=> sprintf( __( 'Edit %s', 'radium' ), $singular ),
			'new_item' 				=> sprintf( __( 'New %s', 'radium' ), $singular ),
			'all_items' 			=> sprintf( __( 'Widget Areas', 'radium' ), $plural ),
			'view_item' 			=> sprintf( __( 'View %s', 'radium' ), $singular ),
			'search_items' 			=> sprintf( __( 'Search %a', 'radium' ), $plural ),
			'not_found' 			=> sprintf( __( 'No %s Found', 'radium' ), $plural ),
			'not_found_in_trash' 	=> sprintf( __( 'No %s Found In Trash', 'radium' ), $plural ),
			'parent_item_colon' 	=> '',
			'menu_name' 			=> $plural
		);
			...
			case 'sidebar_to_replace':
				...
				if ( isset( $meta['_sidebar_to_replace'] ) && ( $meta['_sidebar_to_replace'][0] != '' ) ) {
					...
					if ( isset( $sidebars[$value] ) ) {
						...
						$value .= '<br /><strong>' . __( '(Not in use by current theme)', 'radium' ) . '</strong>';
					}
				}
	...
	public function register_custom_column_headings ( $defaults ) {
		...
		$new_columns = array( 'sidebar_to_replace' => __( 'Sidebar To Replace', 'radium' ), 'condition' => __( 'Condition(s)', 'radium' ) );
	...
	public function meta_box_setup () {		
		add_meta_box( 'sidebar-to-replace', __( 'Sidebar To Replace', 'radium' ), array( &$this, 'meta_box_content' ), $this->token, 'side', 'low' );
		...
		add_meta_box( 'sidebar-description', __( 'Description', 'radium' ), array( &$this, 'description_meta_box' ), $this->token, 'normal', 'core' );
	} // End meta_box_setup()
	...
	public function meta_box_content () {
		...
		if ( count( $sidebars ) > 0 ) {
			...
			$html .= '<p>' . __( 'No sidebars are available with this theme.', 'radium' ) . '</p>';
		}
	...
	?>
	<label class="screen-reader-text" for="excerpt"><?php _e( 'Description', 'radium' ); ?></label><textarea rows="1" cols="40" name="excerpt" tabindex="6" id="excerpt"><?php echo $post->post_excerpt; // textarea_escaped ?></textarea>
	<p><?php printf( __( 'Add an optional description, to be displayed when adding widgets to this sidebar area on the %sWidgets%s screen.', 'radium' ), '<a href="' . esc_url( admin_url( 'widgets.php' ) ) . '">', '</a>' ); ?></p>
	<?php
	...
	public function enter_title_here ( $title ) {
		if ( get_post_type() == $this->token ) {
			$title = __( 'Enter sidebar area name here', 'radium' );
		}
		return $title;
	...
	public function update_messages ( $messages ) {
		...
		$messages[$this->token][1] = __( 'Sidebar Area updated.', 'radium' );			
	...
	public function enqueue_styles () {
		...
		if ( in_array( $pagenow, array( 'edit.php', 'post.php', 'post-new.php' ) ) ) {
			...
			wp_register_style( $this->token . '-admin', RADIUM_ADMIN_URL . '/sidebars/assets/css/sidebars_admin.css', array(), '1.0.0' );
		...
		if ( in_array( $pagenow, array( 'edit.php' ) ) ) {
			wp_register_style( $this->token . '-admin-posts', RADIUM_ADMIN_URL . '/sidebars/assets/css/sidebars_admin_posts.css', array(), '1.0.0' );
	...
	public function add_post_column_headings ( $defaults ) {
		$defaults['radiumsidebars_enable'] = __( 'Custom Sidebars', 'radium' );
		return $defaults;
	...
	public function enable_custom_post_sidebars () {
		...
		if( ! current_user_can( 'edit_posts' ) ) wp_die( __( 'You do not have sufficient permissions to access this page.', 'radium' ) );
		
		if( ! check_admin_referer( 'radiumsidebars-post-enable' ) ) wp_die( __( 'You have taken too long. Please go back and retry.', 'radium' ) );
	...
	public function register_plugin_version () {
		if ( $this->version != '' ) {
			update_option( 'radium' . '-version', $this->version );
		}
