...
function radium_register_post_type_slider() {
	...
	$args = array(
		...
		'menu_position' => null,
		'menu_icon' => RADIUM_ADMIN_URL . '/slider/images/icon-slider.png'
	);
		...
		if( have_posts() ) while ( have_posts() ) : the_post();
			...
			$slidertype = radium_get_custom_field('_radium_slider_type', $post->ID ); 
  			...
  			if ( $slidertype === "content-carousel" ) {
  				...
  				$output = radium_load_template_part(RADIUM_SLIDER_LOCATION.'/slider', 'content-carousel' );
  				 
  			} elseif ( $slidertype === "featured-content" ) {
  		
  				$output = radium_load_template_part(RADIUM_SLIDER_LOCATION.'/slider', 'featured-content' );
  			
  			} else {
  				
  				$output = radium_load_template_part(RADIUM_SLIDER_LOCATION.'/slider', $slidertype );
...
function radium_slider_media_button($context) {
	...
	if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) ) {
		...
		$title = esc_attr( __( 'Add a Slider', 'radium' ) );
 		$img 	= '<img src="' . esc_url( RADIUM_ADMIN_URL ) . '/slider/images/icon-slider1.png" alt="' . $title . '" width="15" height="15" />';
...
function radium_slider_admin_footer() {
	...
	$plugin_icon_url = esc_url( RADIUM_ADMIN_URL );
	...
	if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) ) {
		...
		<div id="choose-radium-slider" style="display: none;">
			<div class="wrap" style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif;">
				<div id="icon-radium" class="icon32" style="background: url(<?php echo $plugin_icon_url.'/slider/images/icon-slider1.png'; ?>) no-repeat scroll 0 50%; width: 16px;"><br></div>
				...
				<a id="radium-cancel-slider" class="button-secondary" onclick="tb_remove();" title="<?php echo esc_attr( 'Cancel' ); ?>"><?php _e('Cancel' , 'radium'); ?></a>
...
function radium_slider_atachment_fields($form_fields, $post) {
	...
	if ( 'slider' == $post_type || 'slider' == $attachment_post_type ) {
	    ...
	    $form_fields['buttons'] = array(
	        ...
	        'html' => "<input type='submit' class='button' name='send[$post->ID]' value='" . esc_attr__( 'Insert into Slider' ) . "' />",
	        'input' => 'html'
	    );
	}
