...
/* Added Truncation Parameter for long single post titles
/**
 ...
 * @return string
 */
function radium_breadcrumbs( $args = '' ) {

	/* Set up the default arguments for the breadcrumb. */
	$defaults = array(
		'prefix' => '',
		'suffix' => '',
		'title' => __( 'You are Here: ', 'radium' ),
		'home' => __( 'Home', 'radium' ),
		'separator' => '',
		'front_page' => false,
		'show_blog' => false,
	...
	$args = apply_filters( 'radium_breadcrumbs_args', $args );

	$args = wp_parse_args( $args, $defaults );

	if ( is_front_page() && !$args['front_page'] )
		return apply_filters( 'radium_breadcrumbs', false );
	...
	/* Get the items. */
	$items = radium_breadcrumbs_get_items( $args );
	...
	$breadcrumbs = apply_filters( 'radium_breadcrumbs', $breadcrumbs );
...
/**
 * Gets the items for the breadcrumbs .
 *
 * @since 0.4
 */
function radium_breadcrumbs_get_items( $args ) {
	global $wp_query;
	...
	if ( function_exists( 'is_bbpress' ) && is_bbpress() )
		$item = array_merge( $item, radium_breadcrumbs_get_bbpress_items() );

	/* If viewing a home/post page. */
	elseif ( is_home() ) {
		$home_page = get_page( $wp_query->get_queried_object_id() );
		$item = array_merge( $item, radium_breadcrumbs_get_parents( $home_page->post_parent ) );
	...
	elseif ( is_singular() ) {
		...
		if ( 'page' !== $wp_query->post->post_type ) {
			...
			if ( isset( $args["singular_{$wp_query->post->post_type}_taxonomy"] ) && is_taxonomy_hierarchical( $args["singular_{$wp_query->post->post_type}_taxonomy"] ) ) {
				...
				$item = array_merge( $item, radium_breadcrumbs_get_term_parents( $terms[0], $args["singular_{$wp_query->post->post_type}_taxonomy"] ) );
			}
		...
		if ( ( is_post_type_hierarchical( $wp_query->post->post_type ) || 'attachment' === $wp_query->post->post_type ) && $parents = radium_breadcrumbs_get_parents( $wp_query->post->post_parent ) ) {
			$item = array_merge( $item, $parents );
		}

		$item['last'] = trim(substr(get_the_title(), 0, $args['max_length'] ). $args['trail']);
	}
	...
	/* If viewing any type of archive. */
		...
		if ( is_category() || is_tag() || is_tax() ) {
			...
			if ( ( is_taxonomy_hierarchical( $term->taxonomy ) && $term->parent ) && $parents = radium_breadcrumbs_get_term_parents( $term->parent, $term->taxonomy ) )
				$item = array_merge( $item, $parents );
			...
			if ( is_day() )
				$item['last'] = __( 'Archives for ', 'radium' ) . get_the_time( 'F j, Y' );

			elseif ( is_month() )
				$item['last'] = __( 'Archives for ', 'radium' ) . single_month_title( ' ', false );

			elseif ( is_year() )
				$item['last'] = __( 'Archives for ', 'radium' ) . get_the_time( 'Y' );
		}

		else if ( is_author() )
			$item['last'] = __( 'Archives by: ', 'radium' ) . get_the_author_meta( 'display_name', $wp_query->post->post_author );
	}

	/* If viewing search results. */
	else if ( is_search() )
		$item['last'] = __( 'Search results for "', 'radium' ) . stripslashes( strip_tags( get_search_query() ) ) . '"';

	/* If viewing a 404 error page. */
	else if ( is_404() )
		$item['last'] = __( 'Page Not Found!', 'radium' );

	return apply_filters( 'radium_breadcrumbs_items', $item );
}
 ...
 */
function radium_breadcrumbs_get_bbpress_items( $args = array() ) {
	...
	elseif ( bbp_is_single_topic() ) {
		...
		$item = array_merge( $item, radium_breadcrumbs_get_parents( bbp_get_topic_forum_id( $topic_id ) ) );
		...
		if ( bbp_is_topic_split() )
			$item[] = __( 'Split', 'radium' );

		elseif ( bbp_is_topic_merge() )
			$item[] = __( 'Merge', 'radium' );
	...
	elseif ( bbp_is_single_reply() ) {
		...
		$item = array_merge( $item, radium_breadcrumbs_get_parents( bbp_get_reply_topic_id( $reply_id ) ) );
	...
	elseif ( bbp_is_single_forum() ) {
		...
		if ( 0 !== $forum_parent_id)
			$item = array_merge( $item, radium_breadcrumbs_get_parents( $forum_parent_id ) );
	...
	return apply_filters( 'radium_breadcrumbs_get_bbpress_items', $item, $args );
}
 ...
 */
function radium_breadcrumbs_get_parents( $post_id = '', $separator = '/' ) {
 ...
 */
function radium_breadcrumbs_get_term_parents( $parent_id = '', $taxonomy = '', $separator = '/' ) {
