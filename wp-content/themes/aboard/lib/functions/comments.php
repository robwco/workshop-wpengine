...
-----------------------------------------------------------------------------------*/
 function radium_custom_form_filters( $args = array(), $post_id = null ) {
	...
	$fields =  array(
		'author' => '<div class="six columns comment-form-left"><div class="comment-form-author">' . '<label for="author">' . __( 'Name', 'radium' ) . ( $req ? '<span class="required">*</span>' : '' ) . '</label> ' .
		            '<input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></div>',
		'email'  => '<div class="comment-form-email"><label for="email">' . __( 'Email', 'radium' ) . ( $req ? '<span class="required">*</span>' : '' ) . '</label> ' .
		            '<input id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></div>',
		'url'    => '<div class="comment-form-url"><label for="url">' . __( 'Website', 'radium') . '</label>' .
	...
	$defaults = array(
		...
		'comment_field'        => '<div class="comment-form-right"><div id="row-comment" class="row"><div class="twelve columns"><label for="comment">' . _x( 'Comment<span class="required">*</span>', 'noun', 'radium' ) . '</label><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></div></div></div>',
		...
		'id_form'              => 'commentform',
		'id_submit'            => 'submit',
		'title_reply'          => __( 'Leave a Reply', 'radium' ),
		'title_reply_to'       => __( 'Leave a Reply to %s', 'radium' ),
		'cancel_reply_link'    => __( 'Cancel reply', 'radium' ),
		'label_submit'         => __( 'Post Comment', 'radium' ),
	);
