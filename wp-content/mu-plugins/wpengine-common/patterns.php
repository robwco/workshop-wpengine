...
$regex_is_user_agent_bot = 'bot|facebook|crawler|feed|flipboard|slurp|re[ea]der|spider|scoutjet|ssowl|site24|google|news|pubsub|^expo9|^tzogeoagent|jakarta|teoma|scrap(?:[ey]|ing)|siteimprove|nutch|coccoc|wazzup|genieo|thunderstone';
...
class Patterns
{
	...
	// Given an input for rewrite paths, check if regex compiles and return false and error log if the regex doesn't compile
	// or return true if all clear
	static public function check_for_noncompiled_regex_string($rewrite_path) {
	...
	// Given a rewrite path, return false and error log if the regex looks dangerous at all (uses quotes, double quotes, or that end in backslashes)
	// or return true if all clear
	static public function reject_dangerous_regex_string($rewrite_path) {
	...
	static public function is_scary_regex($rewrite, $domain, $pattern, $argmatch, $replacement) {
		...
		foreach ($filterees as $key){
			...
			if (! Patterns::check_for_noncompiled_regex_string($rewrite[$key])) {
				$skip = true;
			}
			if (! Patterns::reject_dangerous_regex_string($rewrite[$key])) {
				$skip = true;
			}
		}
		if (! Patterns::reject_dangerous_regex_string($rewrite[$replacement])) {
			$skip = true;
		}
		return $skip;
	}
